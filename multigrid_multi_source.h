...
class FASMultigrid
{
  ...
  IDX_T * u_idx; //storing the index of u for each term, has rho_num terms in total 
  /**
...
#pragma omp parallel for default(shared) private(i,j,k)
  ...
  REAL_T _evaluateEllipticEquationPt(IDX_T depth, IDX_T i, IDX_T j, IDX_T k)
  {
    ...
    return _laplacian(u, i, j, k, n) + _srcVal(idx, depth_idx, u[idx]);
  }
...
#pragma omp parallel for default(shared) private(i,j,k)
  ...
  bool jac_relax(IDX_T depth, REAL_T norm, REAL_T C, IDX_T p)
  {
	...
	damping_v[idx] =  (damping_v[_gIdx(i+1, j, k, n)] + damping_v[_gIdx(i-1, j, k, n)] +
			  ...
			  damping_v[_gIdx(i, j, k+1, n)] + damping_v[_gIdx(i, j, k-1, n)]
			  - jac_rhs[idx] * dx *dx) /
	  ( 6.0 - _srcValDir(idx, depth_idx, u[idx]) *dx *dx);
 ...
 public:
  ...
  {
    IDX_T i, j, k;
    IDX_T n = _2toPwr(max_depth), depth_idx = _dIdx(max_depth);
    REAL_T K = 0.0, delta_phi = 0.01, dx = (REAL_T)grid_length/(REAL_T)n , Lambda = 0.01;
    if(type == 0) //type == 0 for constant K case
    {
      ...
      {
	IDX_T idx = _gIdx(i, j ,k, n);
	rho_h[0][depth_idx][idx] += - PI * _Pwr2( delta_phi* 2.0 * PI /(grid_length) ) 
	  *    (    _Pwr2(-std::sin(2.0 * PI *(REAL_T)i * dx / grid_length + (REAL_T)std::rand()/RAND_MAX * 2.0 * PI)
			  + std::sin(-2.0 * PI *(REAL_T)i * dx /grid_length +(REAL_T)std::rand()/RAND_MAX * 2.0 * PI) )
		    +  _Pwr2(-std::sin(2.0 * PI *(REAL_T)j * dx / grid_length + (REAL_T)std::rand()/RAND_MAX * 2.0 * PI)
			    + std::sin(-2.0 * PI *(REAL_T)j * dx / grid_length+(REAL_T)std::rand()/RAND_MAX * 2.0 * PI) )

		    +  _Pwr2(-std::sin(2.0 * PI *(REAL_T)k * dx / grid_length + (REAL_T)std::rand()/RAND_MAX * 2.0 * PI)
			     + std::sin(-2.0 * PI *(REAL_T)k * dx /grid_length +(REAL_T)std::rand()/RAND_MAX * 2.0 * PI) )
		    );
      }
      ...
      {
	IDX_T idx = _gIdx(i, j, k, n);
	K += Lambda * dx * dx * dx/ 4.0 +  _Pwr2( delta_phi* 2.0 * PI /(grid_length) ) 
	  *    (    _Pwr2(-std::sin(2.0 * PI *(REAL_T)i * dx / grid_length + (REAL_T)std::rand()/RAND_MAX * 2.0 * PI)
			  + std::sin(-2.0 * PI *(REAL_T)i * dx /grid_length +(REAL_T)std::rand()/RAND_MAX * 2.0 * PI) )
		    +  _Pwr2(-std::sin(2.0 * PI *(REAL_T)j * dx /grid_length + (REAL_T)std::rand()/RAND_MAX * 2.0 * PI)
			     + std::sin(-2.0 * PI *(REAL_T)j * dx /grid_length +(REAL_T)std::rand()/RAND_MAX * 2.0 * PI) )
      ...
      K = - K / (grid_length * grid_length * grid_length);
